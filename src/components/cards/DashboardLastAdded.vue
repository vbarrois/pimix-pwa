...
<template>
  <div class="max-w w-full lg:max-w-full lg:flex">
    <div v-lazy="getLastAddedCover()" class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-xl-bl-sm rounded-tl-md text-center overflow-hidden">
    </div>
    <div class="border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal w-full">
      <div class="mb-0">
        <div class="text-gray-900 font-bold text-xl mb-2">Derniers titres ajoutés</div>
        <div v-for="song in controller.controller.lastadded" :key="song.id" class="truncate">
